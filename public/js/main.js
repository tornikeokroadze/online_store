(function($){
	...
	$elementCarousel = $('.atomic-element-carousel');
		...
		if($elementCarousel.elExists()){
			...
			$elementCarousel.each(function(index, element){
				...
				var $spaceBetween = $options.spaceBetween ? parseInt($options.spaceBetween, 10) : 0,
					...
					$prevArrow = $arrows === true ? ($options.prevArrow ? '<span class="'+ $options.prevArrow.buttonClass +'"><i class="'+ $options.prevArrow.iconClass +'"></i></span>' : '<button class="tty-slick-text-btn tty-slick-text-prev"><i class="dl-icon-left"></span>') : '',
					$nextArrow = $arrows === true ? ($options.nextArrow ? '<span class="'+ $options.nextArrow.buttonClass +'"><i class="'+ $options.nextArrow.iconClass +'"></i></span>' : '<button class="tty-slick-text-btn tty-slick-text-next"><i class="dl-icon-right"></span>') : '',
...
$(document).ready(function(){
  $("#top-comment").addClass("display");
   $("#close-top-comment").click(function(){ 
    event.preventDefault();
    $("#top-comment").addClass("close-top-comment");
  });
});



function cart(product_id, status=null, new_quantity = null){
...
function wishlist(product_id,status=null){
	$.ajax({
		type: "GET",
		url: "/wishlist-save",
		data: {
		...
		success: function(msg){
			if(msg[0]=='error' || msg[0]=='success'){
				...
				$('#wishlistLists').html(msg);
				if ($('#heartIcon_' + product_id).css('color') === 'rgb(255, 0, 0)') {
	                $('#heartIcon_' + product_id).css('color', 'black');
	            } else {
	                $('#heartIcon_' + product_id).css('color', 'red');
	            }
			}
			
			$('#remove-wishlist-item-'+product_id+'').hide();

			if (!$('#wishlist-page').length) {
                $('#miniWish').addClass("open");
            }
...
//wishlist and cart count
    function updateCounts() {
        ...
        $.ajax({
            ...
            url: "/wishlist-count",
            success: function(msg) {
                $('#wishlist-count').text(msg);
                $('#wishlist-count-products').text(msg);
            },
            error: function(err) {
                console.log(err);
            }
	...
	function cart_qty() {
        ...
        $(document).on('change', '.update_quantity', function(e) {
            e.preventDefault();
            var el = $(this);
            var quantity = el.val();
            var dataId = el.closest("tr").attr("data-id");
            ...
            $.ajax({
                ...
                success: function(msg) {
					...
					var totalPrice = parseFloat(msg.price);
			        var totalWrapper = el.closest('tr').find('.product-total-price .price-count');
			        $('.cart_subtotal').html(msg.subtotal);
			        $('.cart_total').html(msg.total);
			        totalWrapper.text(totalPrice);
                }
            });
        });
    }
